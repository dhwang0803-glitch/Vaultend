...
| 확인 | ✅ 응답이 vault 내 노트를 참조했는지 (context 기반) |
| 확인 | ✅ 저장된 파일 이름 버튼이 표시되는지 |
| 확인 | ✅ 파일 버튼 클릭 시 해당 노트로 이동 |
...
| 기대 | 응답 하단에 suggestedTags 표시 (vault 기존 태그 재사용) |
...
| 항목 | 내용 |
|------|------|
| 실행 | Quick Ask → "React와 TypeScript의 관계" |
| 기대 | AI 응답에 [[wikilink]]가 있으면 suggestedLinks에 표시 |
...
| 확인 | ✅ 토큰 수가 BM25-only보다 더 많은 context 반영 |
