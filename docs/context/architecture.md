...
[스케줄 타이머 fire]
  ...
  → smartScheduling && LicensePort.canUseFeature('smart-scheduling') → Pro만 dirty skip
  → RunMaintenanceUseCase.execute()
    ...
    → findDuplicateTags() (태그 중복, 2단계):
      ...
      → Stage 2 — 임베딩 유사도 (opt-in):
        → 모든 canonical 그룹 → callEmbedding() 일괄 호출
...
| 자동 Maintenance | `main.ts` `scheduleMaintenanceIfEnabled()` (생성 시 + 매 tick) | `auto-maintenance` |
| Smart Scheduling | `main.ts` 스케줄러 내부 분기 | `smart-scheduling` |
| 중복 태그 일괄 병합 | `MaintenanceResultView` 개별 Merge + 배치 Merge | `batch-merge-tags` |
