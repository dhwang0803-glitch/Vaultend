...
| 현재 브랜치 | 경로 |
|------------|------|
| `feature/*` | 현재 브랜치에서 커밋 → `development` PR |
| `development` | 현재 브랜치에서 커밋 → 기존/신규 PR (별도 feature 브랜치 생성 안 함) |
...
```bash
git fetch origin
git log HEAD..origin/development --oneline   # base에 내 브랜치에 없는 커밋?
git log origin/development..HEAD --oneline
```

- base에 새 커밋이 있으면 → `git pull origin development` 먼저
...
base는 항상 `development`. 본문은 아래 형식으로 작성 후 `gh pr create`.
...
## 8. 릴리즈 체인 (PR 생성 후 자동 제안)

PR 생성이 완료되면 릴리즈 여부를 묻는다:

```
PR #{N}이 생성되었습니다. 릴리즈를 진행할까요?
체크리스트 검증 → 로컬 Obsidian 테스트 → 머지 → 태그 → 배포

1. 릴리즈 진행 (권장)
2. 나중에 릴리즈 (/release로 수동 실행)
```

사용자가 "릴리즈 진행"을 선택하면 `/release`를 자동 실행한다.

---

## ⛔ 절대 금지 규칙 (명시적 승인 없이 실행 금지)

1. `git push origin main` — base 브랜치 직접 push 금지
2. PR 없이 `main`/`development`에 직접 merge 금지
3. PR 리뷰(Approve) 없이 merge 금지
4. 다른 브랜치 폴더 파일을 현재 커밋에 포함 금지
5. 요청하지 않은 파일을 추가로 커밋·push 금지
6. 일상 PR을 `main`에 직접 올리지 않는다 — base는 `development`
